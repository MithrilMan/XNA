...
namespace TomShane.Neoforce.Controls {
   ...
   public class TabControl : Container {
      ...
      protected override void DrawControl(Renderer renderer, Rectangle rect, GameTime gameTime) {
         SkinLayer tbcHeaderLayer = Skin.Layers["Header"];
         SkinLayer l1 = Skin.Layers["Control"];
         SkinLayer headerSkin = Skin.Layers["Header"];
         ...
         if (tabPages.Count > 0) {
            ...
            Rectangle prev = new Rectangle(rect.Left, rect.Top + headerSkin.OffsetY, 0, headerSkin.Height);
            for (int i = 0; i < tabPages.Count; i++) {
               SpriteFont font = headerSkin.Text.Font.Resource;
               Margins margins = headerSkin.ContentMargins;
               Point offset = new Point(headerSkin.OffsetX, headerSkin.OffsetY);
            ...
            for (int i = tabPages.Count - 1; i >= 0; i--) {
               headerSkin = tabPages[i].Skin.Layers["Header"] == null ? tbcHeaderLayer : tabPages[i].Skin.Layers["Header"];

               int li = tabPages[i].Enabled ? headerSkin.States.Enabled.Index : headerSkin.States.Disabled.Index;
               Color lc = tabPages[i].Enabled ? headerSkin.Text.Colors.Enabled : headerSkin.Text.Colors.Disabled;
               if (i == hoveredIndex) {
                  li = headerSkin.States.Hovered.Index;
                  lc = headerSkin.Text.Colors.Hovered;
               }

               if (i != selectedIndex) {
                  Margins m = headerSkin.ContentMargins;
                  ...
                  renderer.DrawLayer(headerSkin, rx, col, li);
                  renderer.DrawString(headerSkin.Text.Font.Resource, tabPages[i].Text, sx, lc, headerSkin.Text.Alignment);
               }
            }

            headerSkin = tabPages[selectedIndex].Skin.Layers["Header"] == null ? tbcHeaderLayer : tabPages[selectedIndex].Skin.Layers["Header"];
            Margins mi = headerSkin.ContentMargins;
            ...
            renderer.DrawLayer(headerSkin, ri, col, headerSkin.States.Focused.Index);
            renderer.DrawString(headerSkin.Text.Font.Resource, tabPages[selectedIndex].Text, si, headerSkin.Text.Colors.Focused, headerSkin.Text.Alignment, headerSkin.Text.OffsetX, headerSkin.Text.OffsetY, false);
         }
      }
      ////////////////////////////////////////////////////////////////////////////     
